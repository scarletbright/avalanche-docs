...
On receiving a `Peers` message, a node should compare the nodes appearing
in the message to its own list of neighbors, and forge connections to any
new nodes.

### When Peers is sent

`Peers` messages do not need to be sent in response to a `GetPeers` message, and are sent periodically to announce newly arriving nodes.
The default period for such push gossip is 60 seconds.
