...
Operations have two possible types: `NFTMintOp` and `NFTTransferOp`.
...
#### NFT Mint Op Identifier

The `OpID` for an NFT mint operation is `0x0000000c`.
...
message NFTMintOp {
    ...
    repeated uint32 address_indices = 2; // 04 bytes + size(address_indices)
